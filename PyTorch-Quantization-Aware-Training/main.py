...
### Link : https://leimao.github.io/blog/PyTorch-Quantization-Aware-Training/
############################################################################
...
def prepare_dataloader(num_workers=1, train_batch_size=32, eval_batch_size=32):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=train_batch_size, sampler=train_sampler,
                                               num_workers=num_workers)
    test_loader = torch.utils.data.DataLoader(dataset=test_set, batch_size=eval_batch_size, sampler=test_sampler,
                                              num_workers=num_workers)
...
def measure_inference_latency(
        model, device, input_size=(1, 3, 32, 32),
...
class QuantizedResNet18(nn.Module):
    ...
    def forward(self, x):
        # manually specify where tensor will be converted from floating
        # points to quantized in the quantized model
        x = self.quant(x)
        x = self.model_fp32(x)
...
##**************************************************************************************************************************************##
    ...
    ############################################################################
    #           load weight file in the model
    ############################################################################

    model = load_model(model=model, model_filepath=model_filepath, device=cuda_device)
    model.to(cpu_device)  # to cpu
    fused_model = copy.deepcopy(model)  # Make a copy of the model for layer fusion
    ...
    ####################################################################################
    # input ----> torch.quantization.QuantStub()
    # output --> torch.quantization.DeQuantStub()
    ...
    ##################################################################################################
    #  quantization configuration  (ex. symmetric quantization, asymmetric quantization)
